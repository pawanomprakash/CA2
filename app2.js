...
tilefront.forEach((item, index) => {
 ...
 tilefront.forEach((item)=>{
    item.addEventListener('contextmenu',(event)=>{
        ...
        if(item.style.backgroundImage==="url(Assets/Rectangle14.svg)"){
            item.style.backgroundImage="url(Assets/tilebackground.svg)"
        }else{
        item.style.backgroundImage="url(Assets/Rectangle14.svg)"
 }})
 })
